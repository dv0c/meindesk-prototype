...
import { SerializedEditorState, LexicalEditor, $getRoot } from 'lexical'
...
export default function PageEditor({ params }: EditorPageProps) {
  ...
  const handleSave = async () => {
    if (!pageId || !siteId || !editorInstance) return
  ...
  const unsavedChanges = useMemo(() => {
    ...
    const contentChanged =
      JSON.stringify(editorState) !== JSON.stringify(page.content)
          ...
          <div className='flex items-center gap-3 flex-wrap'>
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                ...
                <DropdownMenuItem
                  className='cursor-pointer'
                  onClick={() => handleStatus({ status: 'DRAFT' })}
                >
                  Draft
                </DropdownMenuItem>
                <DropdownMenuItem
                  className='cursor-pointer'
                  onClick={() => handleStatus({ status: 'ARCHIVED' })}
                >
                  Archived
                </DropdownMenuItem>
              </DropdownMenuContent>
