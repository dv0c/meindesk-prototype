...
import { createItem, getItems } from "@/lib/actions/item-actions"
    ...
    useEffect(() => {
        const loadCollection = async () => {
            ...
            if (res.collection) {
                ...
                res.collection.fields.forEach((f: any) => {
                    if (f.type === 'boolean') initialData[f.name] = false
                })
                setFormData(initialData)

                // Fetch Related Items
                const relationFields = res.collection.fields.filter((f: any) => f.type === 'relation' && f.relatedCollectionId)
    ...
    const handleSubmit = () => {
        ...
        startTransition(async () => {
            const res = await createItem({
                ...
                data: formData,
                status: "PUBLISHED" // Default to published for simplicity V1
            })

            if (res.success) {
                toast.success("SYSTEM UPDATE: ITEM CREATED")
    ...
    if (loading) return (
        <div className="fixed inset-0 bg-background flex items-center justify-center font-mono">
            <div className="flex flex-col items-center gap-4">
                ...
                <ScrambleTextOnHover text="LOADING SYSTEM..." />
            </div>
        </div>
    )
                ...
                <div className="max-w-3xl mx-auto flex flex-col items-center">
                    ...
                    >
                        <h1 className="font-[var(--font-bebas)] text-3xl md:text-5xl text-center mb-4 tracking-wide text-foreground/80 uppercase">
                            NEW {collection.name?.slice(0, -1) || "ITEM"}
                        </h1>
                ...
                <div className="px-4 md:px-8 h-16 md:h-20 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="flex gap-1">
                            {/* Static progress bar since it's one step */}
                    ...
                    <div className="flex gap-4">
                        ...
                        >
                            <span className="relative z-10 flex items-center gap-2">
                                <ScrambleTextOnHover
                                    text={isPending ? "PROCESSING..." : "COMMIT ENTRY"}
                                    as="span"
                                    duration={0.3}
                                />
