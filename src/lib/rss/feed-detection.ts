...
/**
 * Helper function to parse XML/RSS/Atom/JSON feeds
 * Used when the target URL itself is already a feed (not an HTML page)
 */
async function parseXmlFeed(
  ...
  try {
    ...
    console.error(`parseXmlFeed error for ${feedUrl}:`, err.message);
  }
  return null;
}
...
export async function detectRealFeed(
  ...
  // --- Early check: Is the target URL itself already an RSS/XML feed? ---
  // Check content-type or if the response starts with XML markers
  const isXmlResponse = contentType.includes("xml") ||
  ...
  if (isXmlResponse) {
    console.error(`[RSS Scraper]: Target URL is already an XML/RSS feed, parsing directly`);
    // Try to parse as feed directly
    const feedResult = await parseXmlFeed(target, text, contentType, fetchContent, siteMeta, maxItems, baseUrl);
    if (feedResult) return feedResult;
  }

  // --- Otherwise, parse as HTML to find feed links ---
